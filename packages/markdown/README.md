...
The target is to read and parse data from markdown file as article. So the input params is a markdown file (file content / the absolute path with filename), and the output should be parsing data, including:
...
- Add `id: string` instead of filename detecting (to simplify markdown processing chain, no need to add filename detection)
