...
function generateLinkSuffix(metadata, filename, option) {
  ...
  return [linkPrefix, created.getFullYear(), created.getMonth(), created.getDate(), filename].join('/');
}
...
function getCatalogue(tokens) {
  ...
  _.each(tokens, function(token) {
    if (_.isEqual(token.type, TYPE_HEADING)) {
      ...
      // id suppose to suppose chinese using generator
      let preGenerateId = token.text.toLowerCase().replace(/[^\w]+/g, '-');
      if (_.find(existingIds, preGenerateId)) {
        catalogueToken.id = preGenerateId + existingIds.length;
      }
      else {
        catalogueToken.id = preGenerateId;
      }
